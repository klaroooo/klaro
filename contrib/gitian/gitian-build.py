...
gsigs = 'https://github.com/klaro-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('klaro'):
        subprocess.check_call(['git', 'clone', args.url, 'klaro'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/klaro/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'klaro='+args.commit, '--url', 'klaro='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/klaro-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/klaro/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/klaro/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/klaro-project/klaro', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/klaro', exist_ok=True)
    os.chdir('builder/inputs/klaro')
    if args.pull:
